// The demo stores one task list in this browser's Local Storage, so anyone can try it immediately.
const AUTH_SESSION_KEY = "simple-todo-current-user";
const STORAGE_KEY = "simple-todo-tasks";

// Keep a person's existing tasks when moving from the earlier signed-in version
// to this open demo. The copy only runs when the demo list does not exist yet.
migrateLegacyTasks();
...
function migrateLegacyTasks() {
  ...
  const previousStorageKey = previousUser?.id ? `simple-todo-tasks-${previousUser.id}` : null;
  const previousTasks = previousStorageKey ? localStorage.getItem(previousStorageKey) : null;

  if (previousTasks) {
    localStorage.setItem(STORAGE_KEY, previousTasks);
  }
}

function populateProfile() {
  const nameOrEmail = "Demo user";
...
function createTaskElement(task) {
  // A template lets us define the list-item HTML once in app.html.
